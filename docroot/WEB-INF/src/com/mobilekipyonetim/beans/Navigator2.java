...
import com.mobilekipyonetim.util.CityCurfTest;
import com.mobilekipyonetim.util.CityCurfUtil;
import com.mobilekipyonetim.util.GeoUtil;
import com.mobilekipyonetim.util.NameValuePair;
import com.mobilekipyonetim.util.OnlineCoordinateConverter;
...
public class Navigator2 extends BaseController implements Serializable {
	...
	OrderPriority priority = new OrderPriorityImpl();
...
//    }
	...
	public void showOneMeskunMahal(){
		try {
			...
			List<Poi> list = PoiLocalServiceUtil.findPoiByNameAndOrganization("%"+meskunMahal+"%", getOrganizationId(FacesContext.getCurrentInstance()));
			Poi poi = list.get(0);
	...
	public void visualiteMeskunMahal(){
		try {
			...
			//List<Poi> list =getServiceProvider().getPersistanceService().getPoiList();
				List<Poi> list = PoiLocalServiceUtil.findPoiListOrganizationId(getOrganizationId(FacesContext.getCurrentInstance()));
			for (Iterator iterator = list.iterator(); iterator.hasNext();) {
				Poi poi = (Poi) iterator.next();
				Marker marker = new Marker(new LatLng(Double.parseDouble(poi.getLat()), Double.parseDouble(poi.getLng())),"", "",blueDot);
				marker.setTitle("Meskun Mahal " + poi.getName()		);
...
public void sendCoordinateImmediately() throws Exception{
	...
	public void sendCoordinate() throws Exception{
		try {
			...
			order.setVehicleId((long)vehicle.getPrimaryKey());;
			order.setUserId(getUser(FacesContext.getCurrentInstance()).getPrimaryKey());
			...
			order.setStatusId(Util.ORDER_STATUS_NEW);
			order.setPriorityId((long)priority.getPrimaryKey());
			...
			Order tempOrder = OrderLocalServiceUtil.getOrder(order.getPrimaryKey());
